...
<div class="worklog">
    <form action="{{ route('leaves.store') }}" method="POST">
        ...
        <section class="worklog_scroll">
            ...
            <div>
                <button class="time_calculate" onclick="daysCalculator()">Calculate Days</button>
            </div>

            <div class="mb-3">
                <label for="exampleInputEmail1">Total Days</label>
                <input type="text" class="form-control" id="totalDays" aria-describedby="emailHelp"
                    value="{{ old('total_days') }}" name="total_days" readonly>
                @error('total_days')
                    <p class="text-danger small"><small>{{ $message }}</small></p>
                @enderror
            </div>
